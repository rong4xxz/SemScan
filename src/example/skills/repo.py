"""
Repository read operations: RepoSkills, FileHit. Return dict to support pipeline $ref.
"""
...
class RepoSkills:
    """
    Repository I/O + text search. Methods return dict, to support pipeline $ref and evidence extraction.
    When an error occurs, throw RuntimeError("Error: reason"), and the pipeline writes results[step_id].error.
    """
    ...
    def repo_tree(self, *, max_entries: int = 400) -> Dict[str, object]:
        ...
        for p in self.iter_source_files():
            try:
                ...
                raise RuntimeError(f"Error: failed to get relative path for path={p}: {e}")
            out.append(rel_path)
    ...
    def list_dir(
        ...
        *,
        ...
        if self.root not in abs_dir.parents and abs_dir != self.root:
            raise RuntimeError(f"Error: list_dir path out of bounds: {rel_dir}")
        if not abs_dir.exists():
            raise RuntimeError(f"Error: list_dir path does not exist: {rel_dir}")
        if not abs_dir.is_dir():
            raise RuntimeError(f"Error: list_dir is not a directory: {rel_dir}")
        try:
            children = sorted(abs_dir.iterdir(), key=lambda p: (not p.is_dir(), p.name.lower()))
        except OSError as e:
            raise RuntimeError(f"Error: list_dir cannot read directory {rel_dir}: {e}")
        out: List[str] = []
        for p in children:
            ...
            try:
                ...
                raise RuntimeError(f"Error: list_dir failed to get relative path: {e}")
            out.append(rel)
    ...
    def read_head(self, *, rel_path: str, n: int = 120) -> Dict[str, object]:
        ...
        if self.root not in abs_path.parents and abs_path != self.root:
            raise RuntimeError(f"Error: read_head path out of bounds: {rel_path}")
    ...
    def read_range(self, *, rel_path: str, start_line: int, end_line: int) -> Dict[str, object]:
        abs_path = (self.root / rel_path).resolve()
        if self.root not in abs_path.parents and abs_path != self.root:
            raise RuntimeError(f"Error: read_range path out of bounds: {rel_path}")
        if not abs_path.exists():
