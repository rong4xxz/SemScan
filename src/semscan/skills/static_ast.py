"""
Static analysis based on Python's built-in ast module (Python only).
Methods return dict objects so pipeline $ref can use fields such as
``s0.path``, ``s0.start_line``, and ``s0.end_line``.
"""
...
class AstStaticSkills:
    """
    AST-based static analysis (Python only). Supports build_index,
    locate_class_or_method, and locate_method_by_line.
    Public methods return dict objects for pipeline $ref and result storage.
    """
    ...
    def _parse_file(self, *, abs_path: Path) -> Tuple[str, List[SymbolSpec]]:
        ...
        try:
            ...
            raise RuntimeError(f"Error: path is not inside repo_root: {abs_path}")
        try:
            src = self._read_file(abs_path)
            tree = ast.parse(src, filename=rel_path)
        except SyntaxError as e:
            raise RuntimeError(f"Error: parse failed (SyntaxError) {rel_path}:{getattr(e, 'lineno', '?')} {e.msg}")
        except Exception as e:
            raise RuntimeError(f"Error: parse failed ({type(e).__name__}) {rel_path}: {e}")
    ...
    def locate_class_or_method(
        ...
        *,
        ...
        """Return a dict with ok, hits, and the first hit's path/start_line/end_line for $ref use."""
        if not self._built:
            self.build_index()
        ...
        if class_name and method_name:
            ...
            if not hits_list:
                raise RuntimeError(f"Error: definition for {class_name}.{method_name} was not found")
        elif class_name:
            hits_list = self._classes_by_name.get(class_name, [])
            if not hits_list:
                hits_list = self._methods_by_name.get(class_name, [])
            if not hits_list:
                raise RuntimeError(f"Error: definition for class {class_name} was not found")
        elif method_name:
            hits_list = self._methods_by_name.get(method_name, [])
            if not hits_list:
                hits_list = self._classes_by_name.get(method_name, [])
            if not hits_list:
                raise RuntimeError(f"Error: definition for method {method_name} was not found")
        else:
            raise RuntimeError("Error: class_name or method_name must be provided")
    ...
    def locate_method_by_line(self, *, rel_path: str, line: int) -> Dict[str, Any]:
        """Return a dict with path, start_line, end_line (for $ref), plus found/ok."""
        rel_path = str(rel_path).replace("\\", "/").lstrip("/")
        abs_path = (self.root / rel_path).resolve()
        if not abs_path.exists():
            raise RuntimeError(f"Error: file {rel_path} was not found")
...
StaticSkills = AstStaticSkills  # Compatibility alias
