"""
Dataclasses and I/O helpers: LocationSpec, AgentFinding, load_locations_*,
safe_json_loads, and finding_to_jsonable.
Migrated from semscan.schemas into the new semscan architecture.
"""
...
class LocationSpec:
    """Input location description for a source/sink: path, line, symbol?, snippet?, note?"""
    path: str
    line: int
    ...
    def from_dict(d: Dict[str, Any]) -> "LocationSpec":
        if "path" not in d or "line" not in d:
            raise ValueError("LocationSpec must include path and line")
        return LocationSpec(
            path=str(d["path"]),
            line=int(d["line"]),
...
@dataclass(frozen=True)
class QuerySpec:
    """Structured query entry used inside next_queries."""
    ...
    def from_dict(d: Dict[str, Any]) -> "QuerySpec":
        if "type" not in d or "symbol" not in d:
            raise ValueError("QuerySpec must include type and symbol")
        return QuerySpec(
            type=str(d["type"]),
            symbol=str(d["symbol"]),
...
@dataclass(frozen=True)
class AgentFinding:
    """Structured output produced by a worker agent."""
    role: str
    claim: str
    confidence: float
...
def load_locations_jsonl(text: str) -> List[LocationSpec]:
    ...
    for idx, raw in enumerate(text.splitlines(), start=1):
        ...
        try:
            ...
            raise ValueError(f"Line {idx} in JSONL is not valid JSON: {raw}") from e
...
def load_locations_yaml(text: str) -> List[LocationSpec]:
    """
    Parse YAML rules and load locations from ``data["rules"]``.
    Strips the repository-name prefix from ``path`` (the first path segment),
    matching semscan behavior.
    Dependency: PyYAML (``pip install pyyaml``).
    """
    try:
        import yaml
    except ImportError as e:
        raise ImportError("load_locations_yaml requires PyYAML; run `pip install pyyaml`") from e
    data = yaml.load(text, Loader=yaml.FullLoader)
    if not data or "rules" not in data:
        return []

    def _strip_repo_prefix(item: Dict[str, Any]) -> Dict[str, Any]:
        """Strip the repository-name prefix from path, e.g. ``repo/utils/...`` -> ``utils/...``."""
...
def safe_json_loads(s: str) -> Optional[Dict[str, Any]]:
    """Extract a JSON object from model output: try full parsing first, then the first ``{...}`` block."""
    s = s.strip()
    if not s:
        return None
    try:
...
def finding_to_jsonable(f: AgentFinding) -> Dict[str, Any]:
    """Convert a finding to a JSON-serializable shape for the orchestrator report."""
    return {
        "role": f.role,
        "claim": f.claim,
